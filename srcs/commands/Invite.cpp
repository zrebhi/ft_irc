...
void Command::invite()
{
	...
	if (_commandArray.size() < 3)
		return (void)ft_send(this->_client, ERR_NEEDMOREPARAMS(this->_client, "INVITE"));
	...
	if (allClientsIt == _ircServ.getClientList().end())
		return (void)ft_send(this->_client, ERR_NOSUCHCHANNEL(this->_client, "no user")); //change message to no such user
	std::cout << "four" << std::endl;
	if (channel.getUsers().find(ClientName) != channel.getUsers().end())
		return (void)ft_send(this->_client, ERR_NOSUCHCHANNEL(this->_client, "is logged")); //change message to already log
